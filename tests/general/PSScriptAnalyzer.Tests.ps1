...
Param (
	...
	[string[]]
	$CommandPath = @("$global:testroot\..\MSGraphPermissions\functions", "$global:testroot\..\MSGraphPermissions\internal\functions")
)
...
Describe 'Invoking PSScriptAnalyzer against commandbase' {
	...
	Context "Analyzing <BaseName>" -ForEach $commandFiles {
		BeforeAll {
			$settingsPath = "$global:testroot\..\MSGraphPermissions\PSScriptAnalyzerSettings.psd1"
			if (Test-Path $settingsPath) {
				$analysis = Invoke-ScriptAnalyzer -Path $FullName -Settings $settingsPath
			} else {
				$analysis = Invoke-ScriptAnalyzer -Path $FullName -ExcludeRule PSAvoidTrailingWhitespace, PSShouldProcess, PSUseSingularNouns, PSUseShouldProcessForStateChangingFunctions, PSUseOutputTypeCorrectly
			}
		}
